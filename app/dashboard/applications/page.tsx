...
import {
  ...
  Send, Trash2, ExternalLink, MapPin, Calendar,
} from "lucide-react";
...
function ApplicationsInner() {
  ...
  return (
    <div style={{ height:"100%", display:"flex", flexDirection:"column", background:"#f8fafc" }}>
      <div style={{ background:"#fff", borderBottom:"1px solid #f1f5f9", padding:"20px 28px 16px", flexShrink:0 }}>
        <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:4 }}>
          <ClipboardList size={22} color="#059669" />
          <h1 style={{ fontWeight:900, fontSize:22, color:"#0f172a" }}>Mes candidatures</h1>
        </div>
        <p style={{ fontSize:14, color:"#94a3b8", marginLeft:34 }}>Suis l'état de toutes tes candidatures en un seul endroit</p>
      </div>
      <div style={{ flex:1, overflowY:"auto", padding:"24px 28px" }}>
        ...
        <div style={{ background:"#fefce8", border:"1px solid #fde68a", borderRadius:12, padding:"12px 16px", marginBottom:20, fontSize:13, color:"#92400e" }}>
          <strong>Comment ça marche ?</strong> Crée une candidature quand tu veux postuler, puis marque-la <strong>"Soumise"</strong> une fois que tu as envoyé ton dossier.
        </div>
        {isLoading && (
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(340px,1fr))", gap:12 }}>
            {[1,2,3].map(i => <div key={i} style={{ background:"#fff", borderRadius:16, height:140, border:"1px solid #f1f5f9" }} className="animate-pulse" />)}
          </div>
        )}
        {!isLoading && apps?.length === 0 && (
          <div style={{ textAlign:"center", padding:"80px 20px", background:"#fff", borderRadius:20, border:"1px solid #f1f5f9" }}>
            ...
            <p style={{ fontWeight:800, fontSize:18, color:"#0f172a", marginBottom:8 }}>Aucune candidature</p>
            <p style={{ fontSize:14, color:"#94a3b8", marginBottom:24 }}>Explore le feed et postule à une opportunité qui te correspond.</p>
        ...
        {apps && apps.length > 0 && (
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(340px,1fr))", gap:14 }}>
            {apps.map(app => {
              ...
              return (
                <div key={app.id} style={{ background:"#fff", borderRadius:16, border:"1px solid #f1f5f9", overflow:"hidden", boxShadow:"0 1px 3px rgba(0,0,0,0.04)" }}>
                  ...
                  <div style={{ padding:"16px 18px" }}>
                    <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:10 }}>
                      ...
                      {d !== null && d >= 0 && <span style={{ fontSize:11, fontWeight:700, color:d<=7?"#dc2626":"#94a3b8", display:"flex", alignItems:"center", gap:4 }}><Calendar size={11} />J-{d}</span>}
                      {d !== null && d < 0 && <span style={{ fontSize:11, color:"#94a3b8", fontStyle:"italic" }}>Expirée</span>}
                    </div>
                    <Link href={`/opportunity/${app.opportunity.id}`} style={{ display:"block", fontWeight:700, fontSize:15, color:"#0f172a", textDecoration:"none", lineHeight:1.35, marginBottom:8 }}>{app.opportunity.title}</Link>
                    ...
                    <div style={{ display:"flex", gap:8, borderTop:"1px solid #f8fafc", paddingTop:12 }}>
                      {app.status === "draft" && (
                        <button onClick={() => markSubmitted.mutate(app.id)} disabled={markSubmitted.isPending}
                          style={{ flex:1, display:"flex", alignItems:"center", justifyContent:"center", gap:6, padding:"8px 12px", background:"#059669", color:"#fff", border:"none", borderRadius:10, fontSize:12, fontWeight:700, cursor:"pointer" }}>
                          <Send size={12} />Marquer soumise
                        </button>
                      )}
                      <Link href={`/opportunity/${app.opportunity.id}`} style={{ display:"flex", alignItems:"center", gap:5, padding:"8px 12px", background:"#f8fafc", border:"1px solid #e2e8f0", borderRadius:10, fontSize:12, fontWeight:600, color:"#64748b", textDecoration:"none" }}>
                        <ExternalLink size={12} />Voir
                      </Link>
                      <button onClick={() => { if (window.confirm("Supprimer cette candidature ?")) deleteApp.mutate(app.id); }} disabled={deleteApp.isPending}
                        style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:5, padding:"8px 10px", background:"#fef2f2", border:"1px solid #fecaca", borderRadius:10, fontSize:12, fontWeight:600, color:"#dc2626", cursor:"pointer" }} title="Supprimer">
                        <Trash2 size={13} />
                      </button>
                    </div>
                  </div>
