      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px" }}>
        {isLoading && (
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {[1,2,3].map(i => (
              <div key={i} style={{ background: "#fff", borderRadius: 16, border: "0.5px solid #f3f4f6", padding: 20, height: 100 }}
        ...
        {isError && (
          <div style={{ background: "#fef2f2", border: "0.5px solid #fecaca", borderRadius: 16, padding: 20, color: "#dc2626", fontSize: 14 }}>
        ...
        {!isLoading && data?.length === 0 && (
          <div style={{ textAlign: "center", padding: "80px 20px" }}>
            ...
            <p style={{ fontSize: 14, color: "#9ca3af", marginBottom: 24 }}>
              Sauvegarde des opportunités depuis le feed en cliquant sur 🏷️
            </p>
            <Link href="/dashboard"
                ...
                <div key={opp.id} style={{
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <Link href={`/opportunity/${opp.id}`}
                      style={{ fontWeight: 700, fontSize: 14, color: "#111827", textDecoration: "none", display: "block", marginBottom: 6 }}
                      className="hover:text-emerald-600 transition-colors">
                      {opp.title}
                    </Link>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                      <span style={{ fontSize: 11, fontWeight: 700, color: cfg.color, background: cfg.bg, padding: "2px 8px", borderRadius: 20 }}>
                        {cfg.label}
                      </span>
                      ...
                      <span style={{ fontSize: 11, fontWeight: 800, color: score >= 75 ? "#059669" : "#d97706", marginLeft: "auto" }}>
                        {score}% match
                      </span>
                    </div>
                  </div>

                  <button onClick={() => handleUnsave(opp.id)}
                    title="Retirer des favoris"
                    style={{ background: "none", border: "none", cursor: "pointer", fontSize: 18, color: "#d1d5db",
